...
ENV_FILE="$ENV_FILE" "$EXERCISE_DIR/infrastructure/azure-cli/solution/deploy.sh"

# Deploy code
echo ""
echo "=== Step 2: Deploying Function Code ==="
ENV_FILE="$ENV_FILE" "$EXERCISE_DIR/code/dotnet/deploy.sh"
...
"$EXERCISE_DIR/validate/check-all.sh" || true
